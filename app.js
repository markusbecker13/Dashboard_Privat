...
async function googleSyncDurchfuehren() {
  ...
  try {
    ...
    const geaendertZuPushen = geaendert.filter(t => !inDieserRundeBearbeitet.has(t.id));
    console.log(`[SYNC] Phase 2b: ${geaendertZuPushen.length} geänderte Termine zu pushen (von ${geaendert.length} Kandidaten, ${geaendert.length - geaendertZuPushen.length} bereits durch Phase 1 erledigt) (+${Date.now() - diagStart}ms)`);
    for (const termin of geaendertZuPushen) {
      try {
...
Deno.serve(async (req) => {
  ...
  if (action === "google_sync_cron") {
    ...
    try {
      // Bewusst kein logVerlauf hier: Der Cron-Job läuft alle 15 Minuten
      // und würde den Verlauf-Tab mit "automatisch abgeglichen"-Einträgen
      // fluten und andere Aktivitäten darin unsichtbar machen. Manuelle
      // Syncs über den Button (case "google_sync" unten) werden weiterhin
      // geloggt, da sie eine bewusste Nutzeraktion sind.
  ...
  try {
    switch (action) {
      ...
      case "aufgabe_aktualisieren": {
        ...
        const neuesIntervall = body.erinnere_alle_tage ? parseInt(body.erinnere_alle_tage, 10) : null;
        ...
        const update: Record<string, unknown> = {
          ...
          uhrzeit: body.uhrzeit || null,
          ende_uhrzeit: body.ende_uhrzeit || null,
          erinnere_alle_tage: neuesIntervall,
        };
        // naechste_erinnerung nur neu berechnen, wenn sich das Intervall
        // tatsächlich geändert hat – sonst würde jedes Speichern (auch nur
        // eine Titel-Korrektur) eine laufende Erinnerung auf "heute + X"
        // zurücksetzen und den bisherigen Fortschritt verwerfen.
        if (neuesIntervall !== (bisherige?.erinnere_alle_tage ?? null)) {
          update.naechste_erinnerung = neuesIntervall ? addTage(heuteISO(), neuesIntervall) : null;
        }
      ...
      // Massenlöschung (Buchungen / Fixkosten / Sonderausgaben), gefiltert
      // nach Zeitraum, Typ und Kategorie. "vorschau: true" zählt nur, ohne
      // zu löschen – Frontend zeigt das Ergebnis erst zur Bestätigung an.
      case "finanzen_massenloeschung": {
        const bereiche: string[] = Array.isArray(body.bereiche)
          ? body.bereiche.filter((b: unknown) => ["buchungen", "fixkosten", "sonderausgaben"].includes(String(b)))
          : [];
        if (!bereiche.length) return json({ error: "Kein Bereich ausgewählt" }, 400);

        const von: string | null = body.von || null; // "YYYY-MM-DD"
        const bis: string | null = body.bis || null;
        const typ: string | null = ["ausgabe", "einnahme"].includes(body.typ) ? body.typ : null;
        const kategorie: string | null = body.kategorie ? String(body.kategorie).trim() : null;
        const nurVorschau = body.vorschau !== false; // sicherer Default: zählen statt löschen

        const ergebnis: Record<string, number> = {};

        if (bereiche.includes("buchungen")) {
          let q = supabase.from("buchungen").select("id", { count: "exact", head: true });
          ...
          if (error) throw error;
          ergebnis.buchungen = count ?? 0;
        }

        if (bereiche.includes("fixkosten")) {
          // Fixkosten haben kein Datum (fortlaufende Monatsbeträge) – der
          // Zeitraum-Filter greift hier bewusst nicht, nur Typ/Kategorie.
          let q = supabase.from("fixkosten").select("id", { count: "exact", head: true });
          if (typ) q = q.eq("typ", typ);
          if (kategorie) q = q.eq("kategorie", kategorie);
          const { count, error } = await q;
          if (error) throw error;
          ergebnis.fixkosten = count ?? 0;
        }

        if (bereiche.includes("sonderausgaben")) {
          // Sonderausgaben haben kein "typ"-Feld (immer Einzelposten) –
          // Zeitraum wird über jahr (aus von/bis abgeleitet) gefiltert.
          let q = supabase.from("sonderausgaben").select("id", { count: "exact", head: true });
          if (von) q = q.gte("jahr", parseInt(von.slice(0, 4), 10));
          if (bis) q = q.lte("jahr", parseInt(bis.slice(0, 4), 10));
          if (kategorie) q = q.eq("kategorie", kategorie);
          const { count, error } = await q;
          if (error) throw error;
          ergebnis.sonderausgaben = count ?? 0;
        }

        if (nurVorschau) {
          return json({ ok: true, vorschau: true, anzahl: ergebnis });
        }

        // Tatsächliches Löschen – dieselben Filter wie in der Vorschau.
        const geloescht: Record<string, number> = {};

        if (bereiche.includes("buchungen")) {
          let q = supabase.from("buchungen").delete().select("id");
          if (von) q = q.gte("datum", von);
          if (bis) q = q.lte("datum", bis);
          if (typ) q = q.eq("typ", typ);
          if (kategorie) q = q.eq("kategorie", kategorie);
          ...
          if (error) throw error;
          geloescht.buchungen = data?.length ?? 0;
        }

        if (bereiche.includes("fixkosten")) {
          let q = supabase.from("fixkosten").delete().select("id");
          if (typ) q = q.eq("typ", typ);
          if (kategorie) q = q.eq("kategorie", kategorie);
          const { data, error } = await q;
          if (error) throw error;
          geloescht.fixkosten = data?.length ?? 0;
        }

        if (bereiche.includes("sonderausgaben")) {
          let q = supabase.from("sonderausgaben").delete().select("id");
          if (von) q = q.gte("jahr", parseInt(von.slice(0, 4), 10));
          if (bis) q = q.lte("jahr", parseInt(bis.slice(0, 4), 10));
          if (kategorie) q = q.eq("kategorie", kategorie);
          const { data, error } = await q;
          if (error) throw error;
          geloescht.sonderausgaben = data?.length ?? 0;
        }

        const gesamt = Object.values(geloescht).reduce((s, n) => s + n, 0);
        const filterText = [
          von || bis ? `Zeitraum ${von || "…"} – ${bis || "…"}` : null,
          typ ? `Typ: ${typ}` : null,
          kategorie ? `Kategorie: ${kategorie}` : null,
        ].filter(Boolean).join(", ") || "kein Filter";
        await logVerlauf(
          "Finanzen",
          "Massenlöschung",
          `${gesamt} Einträge gelöscht (${bereiche.join(", ")}) – ${filterText}`
        );

        return json({ ok: true, vorschau: false, anzahl: geloescht });
      }
